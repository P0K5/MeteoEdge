...
### Community 0 - "Bracket"
Cohesion: 0.07
Nodes (34): Rolling residual statistics for a single city (or city+station+source pair)., Correction actually applied (clamped to ±RESIDUAL_MAX_CORRECTION_F)., ResidualStats, WeatherState, db(), _frozen_scanner_now(), _kmia_state(), _make_candidate() (+26 more)
...
Cohesion: 0.06
Nodes (19): analytics_intraday(), Return the intra-day snapshot series for *station* × *date* from analytics.db., ArchiveDatabase, Idempotent schema migrations for analytics.db files created before         a co, Close the underlying SQLite connection., Batch-insert scanner snapshots; idempotent via INSERT OR IGNORE.          Retu, Batch-insert position snapshots; idempotent via INSERT OR IGNORE.          Ret, Return all snapshot_archive rows for *station* on *date*, ordered by ts. (+11 more)
...
Cohesion: 0.09
Nodes (18): fetch_hrrr_hourly(), _kelvin_to_fahrenheit(), Convert temperature from Kelvin to degrees Fahrenheit., Fetch HRRR hourly 2-m temperature forecasts for the next 18 forecast hours., src_data_hrrr, Unit tests for src/data/hrrr.py.  All network calls and GRIB I/O are mocked; t, All 18 forecast hours return a valid temperature., Always return the same Kelvin value regardless of fxx. (+10 more)
...
Cohesion: 0.12
Nodes (14): _connect_ro(), load_gamma_cache(), Open *db_path* read-only. Returns None if missing/unopenable.      Never creat, Load the persistent {ticker: resolved_yes} Gamma cache.      A resolved Polyma, Persist the Gamma cache. Failure is warned, never raised -- see     ``load_gamm, Return ({ticker: resolved_yes}, stats) for every ticker Polymarket has     defi, resolve_gamma_outcomes(), save_gamma_cache() (+6 more)
...
Cohesion: 0.20
Nodes (6): When forecast_ecmwf is None for US station, no warning is emitted., Verify compute_deb_mu_f handles ECMWF/ICON and US-station guard., EU 4-model blend: open_meteo+gfs+ecmwf+icon., Asia 3-model blend: open_meteo+gfs+ecmwf (no icon)., When forecast_ecmwf provided for US station, log warning and exclude., TestComputeDebMuFEcmwfIcon
...
Cohesion: 0.14
Nodes (11): build_consensus(), Return DEB-weighted hourly temperature path for today (UTC).      Renormalises, _make_hourly_data(), Build a minimal Open-Meteo-shaped hourly payload for testing., Only today's slots are returned; values are renormalized over hourly-capable sou, Same input temps must produce same consensus output regardless of weight dict., Returns None when _fetch_open_meteo_hourly returns None., weights nws=0.6, open_meteo=0.4; forecasts nws=82, om=78 -> 0.6*82 + 0.4*78 = 80 (+3 more)
...
Cohesion: 0.24
Nodes (7): load_candidate_directions(), Return {ticker: direction} from the DB ``candidates`` table for the     subset, candidates: list of (ticker, direction, ts) tuples -- minimal shape of     the, A DB that exists but has no `candidates` table (e.g. a synthetic         test f, A ticker denotes one market -- more than one direction on record         should, TestLoadCandidateDirections, _write_candidates_db()
...
Cohesion: 0.28
Nodes (6): _make_archive_class(), Return a drop-in replacement for ArchiveDatabase bound to *db_path*., Endpoint returns non-empty list when the archive has matching rows., Endpoint returns [] (HTTP 200, not 404) for a station with no data., Endpoint works correctly even when the trading DB has no data.          Create, TestAnalyticsIntradayEndpoint
...
Cohesion: 0.20
Nodes (6): Verify compute_deb_mu_f handles 4-model and graceful-degradation cases., 4-model weighted average: nws=0.3, om=0.2, hrrr=0.3, nbm=0.2., When HRRR is None, remaining 3 models renormalise to 1.0., When NBM is None, remaining 3 models renormalise to 1.0., When both HRRR and NBM are None, falls back to 2-model NWS+OM blend., TestComputeDebMuFExtended
