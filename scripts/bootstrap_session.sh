...
def fetch(cursor):
    ...
    try:
        ...
        raise SystemExit(f"[bootstrap] ERROR: gh api call failed with exit code {e.returncode}\nstderr: {e.stderr}")
    except Exception as e:
        raise SystemExit(f"[bootstrap] ERROR: subprocess failed: {e}")

    out = result.stdout
    if out is None:
        raise SystemExit("[bootstrap] ERROR: gh api returned no output")

    try:
        return json.loads(out)["data"]["node"]["items"]
    except (json.JSONDecodeError, KeyError, TypeError) as e:
        raise SystemExit(f"[bootstrap] ERROR: Failed to parse gh output as JSON: {e}")
...
# ---------------------------------------------------------------------------
# 4. Write .claude/session-context.env (atomically)
# ---------------------------------------------------------------------------
...
# Atomically move temp file to final location only after all writes succeed
mv "$TEMP_OUT" "$OUT" || {
    rm -f "$TEMP_OUT"
    echo "[bootstrap] ERROR: Failed to write $OUT" >&2
    exit 1
}
