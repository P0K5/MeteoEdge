...
install -m 0644 "$SRC_DIR/meteoedge-copy-signal.service"          "$UNIT_DIR/meteoedge-copy-signal.service"
...
systemctl enable --now meteoedge-copy-signal.service
...
systemctl --no-pager status meteoedge.service meteoedge-copy-signal.service meteoedge-settle.timer meteoedge-archive.timer meteoedge-capture-forecasts.timer meteoedge-prob-cap-report.timer meteoedge-purge-retention.timer meteoedge-copy-screening.timer meteoedge-resolve-outcomes.timer meteoedge-health-report.timer || true
echo
echo "Tail the bot log with:  journalctl -u meteoedge.service -f"
echo "Tail the copy-signal log with:  journalctl -u meteoedge-copy-signal.service -f"
