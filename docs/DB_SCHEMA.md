...
**Purpose:** One layer up from `copy_positions` — REAL (non-paper) copy-trading positions opened from `copy_signals`. Epic H (#1159), issue #1166; live order placement wired in by issue #1167; settlement, wallet-balance reconciliation, ghost-order recovery, and the `filled_stake_usd` column added by epic I issue #1174.

**Writer:** `src/execution/copy_live_executor.py` + `src/scripts/copy_signal_loop.py` (submission/fill/rejection, issue #1167) / `Database.settle_copy_live_position` (settlement — issue #1174, called by `src/scripts/copy_live_settle.py`, run hourly by `meteoedge-copy-live-settle.timer`) / `Database.update_copy_live_position_status` (also used by `recover_ghost_orders` in `copy_live_settle.py` to true up a ghost row to a correct terminal status)
**Reader:** `Database.get_open_copy_live_positions` (issue #1167's live wallet/total exposure checks) / `Database.get_copy_live_realized_pnl_total` and `Database.get_copy_live_realized_pnl_total_for_date` (issue #1175's live-specific realized-P&L circuit breaker, `src/risk/copy_risk_manager.py::allow_live_copy_signal`; `get_copy_live_realized_pnl_total` is also consumed by issue #1174's wallet-balance reconciliation) / `Database.get_unsettled_copy_live_positions` (issue #1174's settlement pass — `status IN ('filled','partial')`, excludes `'pending'` rows with no fill yet) / `Database.get_ghost_order_positions` (issue #1174's ghost-order recovery)
...
| `fill_price` | REAL CHECK(fill_price IS NULL OR (fill_price >= 0 AND fill_price <= 1)) | 0–1 probability | Yes | Nullable until filled. Also populated on a "ghost order" rejection (`rejected_reason='cancel_failed_ghost'`, issue #1174) so a later-discovered fill can be valued |
| `stake_usd` | REAL NOT NULL CHECK(stake_usd > 0) | USD | No | Originally-**intended** amount staked on this position |
| `filled_stake_usd` | REAL | USD | Yes | Issue #1171 item 3 / #1174. The actual USD spent on a confirmed **partial** fill, distinct from `stake_usd` — used for correct P&L instead of the full intended stake. NULL for `'pending'`/`'rejected'` rows and for `'filled'` rows (assumed equal to `stake_usd`); readers should use `COALESCE(filled_stake_usd, stake_usd)` |
...
| `rejected_reason` | TEXT | free text | Yes | Only populated when `status='rejected'`. `'cancel_failed_ghost'` marks an ambiguous failed-cancel row awaiting `recover_ghost_orders`' recheck (issue #1174); `'cancel_confirmed_zero_fill'` marks one recovery confirmed as a genuine, unambiguous rejection. A row recovered to `'filled'`/`'partial'` keeps its stale `rejected_reason` text — `status` is the authoritative field, never read `rejected_reason` without checking `status=='rejected'` first |
| `entry_ts` | TEXT NOT NULL | ISO 8601 timestamp (UTC) | No | Entry/fill timestamp |
| `settled_pnl_usd` | REAL | USD | Yes | Only populated once `status='settled'` |
| `settled_at` | TEXT | ISO 8601 timestamp (UTC) | Yes | Only populated once `status='settled'` |
...
- Fully separate from `copy_positions`/`open_positions`/`trades` per the architecture doc's isolation decision (issue #1100) — FKs only into `copy_signals(id)`, no FK into `copy_positions` or `open_positions`, no shared writes. `copy_live_settle.py` (issue #1174) never reads or writes `copy_positions`/`open_positions`/`trades`, and is a fully separate systemd process/timer from `copy_settle.py`.
- `Database.insert_copy_live_position` (create), `Database.update_copy_live_position_status` (pre-settlement transitions: `pending` → `filled`/`partial`/`rejected`, plus ghost-order recovery transitions), `Database.settle_copy_live_position` (terminal `filled`/`partial` → `settled` transition, mirrors `settle_copy_position`), `Database.get_open_copy_live_positions` (query, `status` NOT IN `('rejected','settled')`), `Database.get_copy_live_realized_pnl_total` / `get_copy_live_realized_pnl_total_for_date` (issue #1175, read-only realized-P&L aggregation), `Database.get_unsettled_copy_live_positions`, and `Database.get_ghost_order_positions` (issue #1174) are the CRUD methods these issues add.
- `update_copy_live_position_status` uses `COALESCE` against the existing row for `order_id`/`fill_price`/`rejected_reason`/`filled_stake_usd` — a call that omits one of them preserves the previously-written value rather than nulling it out (e.g. a later fill call doesn't need to repeat an `order_id` an earlier submission-recording call already wrote).
- **Ghost-order recovery (issue #1171 item 1 / #1174):** when `src.execution.copy_live_executor`'s GTC cancel-after-timeout call itself fails, the order's true fate is unknown (still resting, already filled, or actually cancelled despite the client-side error) — recorded as `status='rejected'`, `rejected_reason='cancel_failed_ghost'` rather than a plain `'timeout'`/`'cancelled'` rejection, so `copy_live_settle.py`'s `recover_ghost_orders` can find and re-verify these rows specifically via `LiveTrader.check_fill`/`get_order_fill_size`, instead of a real fill silently masquerading as a dead rejection forever.
- **Wallet-balance reconciliation (issue #1174):** `copy_live_settle.py`'s `check_wallet_balance_drift` compares `LiveTrader.get_usdc_balance()` against `COPY_LIVE_CAPITAL_USD - (committed stake over unsettled 'filled'/'partial' rows) + (realized P&L over 'settled' rows)`, and `log.critical`s when the drift exceeds `COPY_LIVE_BALANCE_DRIFT_TOLERANCE_USD` (live-editable, `bot_config`). This assumes `COPY_LIVE_CAPITAL_USD` is the real CLOB balance at the moment live copy-trading began and is never adjusted afterward — an operator top-up/withdrawal of the underlying wallet would show up as drift under this model.
